...
class EmbeddingClient:
    ...
    def _init_local_model(self):
        ...
        if 'nomic' in self.model_name.lower():
            print(f"  Max sequence length: 8192 tokens")
            print(f"  Task prefix support: search_query / search_document")
    ...
    def _encode_nomic_api(self, texts: List[str]) -> List[List[float]]:
        ...
        if response.status_code == 404:
            print(f"Got 404, trying with /embeddings endpoint...")
        ...
        if response.status_code == 422:
            print(f"Got 422 (Unprocessable Entity), trying OpenAI format...")
            print(f"Response: {response.text[:500]}")
            return self._encode_openai_api(texts)
        
        # If still failing, try OpenAI format
        if response.status_code == 404:
            print(f"Still 404, trying OpenAI format...")
            return self._encode_openai_api(texts)
...
class AnsibleErrorEmbedder:
    ...
    def __init__(
        ...
        if not self.model_name:
            raise ValueError("Model name must be provided via parameter or EMBEDDINGS_LLM_MODEL_NAME")
    ...
    def create_composite_embeddings(
        ...
        if use_task_prefix:
            print(f"✓ Using task prefix: 'search_document:'")
